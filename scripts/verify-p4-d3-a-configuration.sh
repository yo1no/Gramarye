...
for required_tool in awk bash grep find git jar javap mktemp rm dirname pwd wc; do
...
verify_search_helpers() {
    ...
    is_approved_p9_s3_mr1_production_path \
        ...
        || fail 'P4-D3-A verifier rejected an exact P9-S3-MR1 production path'
    if is_approved_p9_s3_mr1_production_path \
        ...
        fail 'P4-D3-A verifier accepted a prefix-near P9-S3-MR1 production path'
    fi
}
...
is_approved_p9_s3_production_path() {
    case "$1" in
        src/main/java/com/yo1no/gramarye/Gramarye.java | \
