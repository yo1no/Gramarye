# P4-E0 bounded root-audit authority boundary

This compact ledger indexes the P4-E0 research, qualification, and authority decision. The complete
normative contract remains the
[P4 persistence, Attachment, and composition amendment](../codex-spec/18_P4持久化與組合修正案.md);
the broader phase ledger is [P4-0](P4-0-persistence-boundary.md). This page neither replaces the
amendment nor reproduces its 25-counter table.

## Authority and evidence layers

- **SOURCE／PLATFORM FACT** records locked Minecraft 1.21.1 and NeoForge 21.1.241 behavior: current
  DataVersion 3955, vanilla playerdata's unlimited reader, primary／old and DFU ordering, the NBT
...
## Research and qualification closure

- P4-E0-A's read-only review stopped because the prior authority could not derive every required
  offline-ingress, work-vector, heap, truth-selection, and reconciliation decision.
- R1 established the isolated research harness, real filesystem/source-shape fixtures, exact／+1 raw
  root construction, and parent-owned child-process classification.
- R2 explored directory, single-file, NBT-structure, aggregate, root-capture, and combined envelopes.
- R2R regenerated the exploratory evidence from a clean committed revision.
- R2Q-A locked `BALANCED_V0_1536_QUALIFICATION` as a candidate. B1 added the formal runner; B1.1
  separated official, smoke, and work namespaces; B1.2 initialized the locked game version before
  fixture materialization and archived bounded prepare failure; B1.3 repaired the case-04 exact +1
  fixture without changing its qualification ceiling.
- R2Q-B2 completed all 29 formal cases. R2Q-C independently adjudicated identity, vector isolation,
  downstream cutoffs, hashes, and limitations. P4-E0-R2Q is complete.
...
`EXPLORATORY_NON_NORMATIVE`; the amendment, not this artifact directory, owns the adopted product
policy.

| Identity | Value |
| --- | --- |
| Git HEAD | `cd0c9d8d88bd1d5e0d3765e260006fa537f077bb` |
| Git tree | `c10743ffab258627c1a1b1d187cfad8816c29b98` |
| Study ID | `7dfb3c36d09b20549aa104783b75c37f283b8c7b27111bdc91bd2dd3ee27b38f` |
| Profile identity hash | `6a6f4541f4c23b9aefad465eb29ec0420d3a4f635f06b528ca07239a93f99418` |
| Case-plan identity hash | `23408739f292d2a5696c56c39b8b4b3978b3840af383930293efbe6b824f5035` |
| Fixture-root hash | `faf49698edf56d8f0ef7019f467bc647ab3bbe26612cf283d7a409df3be465c3` |
| Run-order hash | `ca31a912c4aef52b38e78b6443c6abb2f88757e50633774689ec601736b5afc6` |

| Official artifact | SHA-256 |
| --- | --- |
| `PROVENANCE.txt` | `3becb7352b873a310f4eef052d9ae566c5f75256ac4983e5763ac0cb15fea9ff` |
| `r2q-case-plan.json` | `e91c76c890456103fe918841fc05ba56a6441791c480ada7a26e9a9e4103f4d7` |
| `r2q-profile.json` | `1a709f0288eb7d5830d4df3277f851706b14be604538ea6624e3c74a88519a51` |
| `runs.jsonl` | `30b831dbcdc60bd3f874090236de267c4ea93d4049595d59592edf3d0ec84cb5` |
| `summary.md` | `87c26c8ac3af8531cbbd416e412201de4cedcc92e0b2b568f5116236d8ce0a61` |
| `SHA256SUMS.txt` | `cb296db6f2aae653a0db2af25b20df4a5107e90096eff9766e40fa2798f24da9` |

The formal set contains one exact-positive case, 25 isolated counter-MAX+1 cases, and three
DataVersion controls. All 29 parent process classifications are `COMPLETED`; qualification is one
`ADMITTED_EXACT`, 25 `REJECTED_EXPECTED_COUNTER`, and three
`REJECTED_EXPECTED_DATA_VERSION`. OOME and timeout counts are both zero.

## Adopted V0 product boundary
...
floor of `1_610_612_736` bytes (1,536 MiB). P4-E0-B.2 makes effective HotSpot `MaxHeapSize`
VM-option bytes the sole normative observation coordinate without changing that floor.
...
required-check configuration remains external governance unknown. At the B.1 closure revision,
this opened only the P4-E1 read-only design review and implementation was not started. That is
superseded historical wording: the renewed review later passed, and the first E1-A attempt then
stopped at the active heap-floor coordinate conflict recorded below.

## P4-E0-B.2 effective-MaxHeapSize authority correction

The approved E1-A work order exposed an active authority conflict: the P4 amendment still made
`Runtime.maxMemory()` normative even though collector-dependent runtime diagnostics can be lower
than HotSpot's effective configured maximum. B.2 corrects documentation only. The sole normative
coordinate is
`HotSpotDiagnosticMXBean -> getVMOption("MaxHeapSize") -> VMOption.value -> strict canonical
base-10 nonnegative long -> effective MaxHeapSize bytes`. The floor remains exactly
`MIN_P4_E_ROOT_AUDIT_MAX_HEAP_SIZE_BYTES = 1_610_612_736` bytes／1,536 MiB. Effective bytes below it
produce `HEAP_FLOOR_NOT_MET`; bytes at or
above it produce `QUALIFIED_FLOOR_PRESENT`. A missing bean／option, noncanonical／negative／overflowing
value, or approved observation API `RuntimeException` produces `HEAP_FLOOR_UNVERIFIABLE`.
`Error` and `OutOfMemoryError` are not caught.

`Runtime.maxMemory()`, `MemoryMXBean` heap max, collector pool max, and pool peak sum remain bounded
diagnostics only. They are never a fallback, override, min／max operand, tolerance input, or pass／fail
...
`-Xmx1535m -XX:+UseG1GC` aligns to the same effective floor and is the qualified
`ALIGNED_TO_FLOOR_CONTROL`, not a negative control. `-Xmx1024m -XX:+UseG1GC` observes
`1_073_741_824` and is the real below-floor process control. A pure injected observation seam owns
the exact floor − 1／floor／floor + 1 comparison because command-line heap alignment cannot reliably
form the byte immediately below the floor.

B.2 changes neither the R2Q `BALANCED_V0_1536_QUALIFICATION` profile, case plan, 25 maxima, floor,
29-case results, sampled peaks, nor the six-file official evidence set; the formal study was not
...
unknown. The external E1-A archive was outside this closure commit. At that closure revision it
could not be restored until the separate closure commit's own remote jobs passed. Those
...
`6f6585b6e0b997bb09d515fc9520b79b0f0d57af`, with `HEAD == origin/main` and zero
ahead／behind. The external archive was
`/private/tmp/gramarye-p4-e1-a-stopped-20260807TYoCp4R`; its manifest SHA-256 was
`c2ed4c95a768230b1bac914d2268e27d04bae08a7c95df17841b036bf7c4c079`. The archive remained
...
to constructing the 25-counter budget, so nonqualified observations cannot start source work. The
locked Java 21 child matrix observed:

```text
G1       -Xmx1536m -> effective 1_610_612_736; runtime diagnostic 1_610_612_736; QUALIFIED_FLOOR_PRESENT
Parallel -Xmx1536m -> effective 1_610_612_736; runtime diagnostic 1_431_830_528; QUALIFIED_FLOOR_PRESENT
Serial   -Xmx1536m -> effective 1_610_612_736; runtime diagnostic 1_556_938_752; QUALIFIED_FLOOR_PRESENT
ZGC      -Xmx1536m -> effective 1_610_612_736; runtime diagnostic 1_610_612_736; QUALIFIED_FLOOR_PRESENT
G1       -Xmx1535m -> effective 1_610_612_736; runtime diagnostic 1_610_612_736; QUALIFIED_FLOOR_PRESENT (control role ALIGNED_TO_FLOOR_CONTROL)
G1       -Xmx1024m -> effective 1_073_741_824; runtime diagnostic 1_073_741_824; HEAP_FLOOR_NOT_MET
```

Qualified children derived `source_work_calls` from the checked sum of all 25 budget counters and
recorded zero. The below-floor child received no qualified capability or budget and reported zero
source work. A nonzero sentinel regression proves the qualified derivation is not a literal. Pure
floor − 1／floor／floor + 1 and malformed／unavailable observation tests passed;
...
## P4-E1-B read-only design review closure

The renewed P4-E1-B read-only design review is PASS. It started from clean `main` commit
`468ac130b2441456c4213b4fc213d62c34316fc2`, tree
`eb6395cbec748e892e67fa23bfa53bd52893811e`, with `HEAD == origin/main`. The prior documentation
closures are P4-E0-B.3 commit `ac28d456780047aa210a3b687b418561faed9ee7`／tree
...
`origin/main`; each exact-SHA workflow contains the five successful `build`, `P4-A3 memory gates`,
`P4-B memory gates`, `P4-C memory gates`, and `P4-D memory gates` jobs.

The review hit no Stop Condition. The amendment now fixes the closed inventory, exact global
checkpoint order, online／integrated／disk arbitration, counter coordinates, freshness rules,
grouped Store-audit boundary, memory-only index restrictions, and E3 handoff. The completed A.1
bridge supplies the required tag-free, single-use P4-C admission projection without widening the
public raw boundary. No authority conflict, new persistent truth, schema change, numeric change,
network surface, offline writer, chunk force, or need for a new amendment was found.

Implementation is forcibly split into P4-E1-B1 followed by P4-E1-B2:

- **P4-E1-B1** owns the compile-time closed inventory and exact provider coverage; bounded online／
  integrated／disk global source arbitration in one UUID-natural owner order; player and journal raw-root
  capture; the `raw_root_claims` capacity check before append and before any deduplication; and the
  directory／file／source plus online／integrated final-freshness witnesses. Its only successful product
  is a package-private, single-use, unpublished capture. The capture may carry a single-use final
  freshness check for B2, but it is not a public audit result, is never stored in the index, and is
  discarded on every terminal failure.
- **P4-E1-B2** alone consumes that capture, performs the grouped exact-reference／expected-owner Store
  audit, invokes the final freshness check after the Store audit, publishes the bounded public audit
...
remained blocked until B1 closure; that prerequisite has closed. E1 as a whole still has zero
player／Store／journal mutation, zero dirty delta, and zero reclaim invocations. E3 alone may convert
a fresh same-call-chain `Complete` handoff to `SkillRetentionRootSnapshot.Complete` and invoke
controlled reclaim exactly once.

The P4-E3 exact `-Xms512m -Xmx1536m -XX:+ExitOnOutOfMemoryError` production-shaped first／restart
Gate remains mandatory and is not waived by this review, the B1／B2 split, R2Q, or any existing
memory job. Branch-protection required-check configuration remains external governance unknown.
This closure changes documentation only and starts neither B1 nor B2 implementation.
...
reclaim at zero; only restart N+1 can obtain a new Complete result.

P4-E3 alone composes `P4-B install -> P4-D bootstrap -> fresh P4-E1 audit -> immediate controlled
reclaim exactly once` on the same logic thread. P4-B retains publication-before-dirty behavior.
Incomplete, over-limit, reconciliation-pending, or unavailable sources never reach reclaim.

## Heap qualification and E3 obligation

The R2Q exact-positive case 00 sampled peak was `1_499_741_840` bytes, leaving `110_870_896` bytes
or `6.883771221%`. The narrowest sampled headroom was case 11
(`long_array_elements_per_file` MAX+1): peak `1_605_137_872`, headroom `5_474_864` bytes or
`0.339924296%`. Case 19 (`byte_array_elements_total` MAX+1) left `8_525_536` bytes or
`0.529334942%`. Sampling may miss transient peaks.

P4-E3 must still pass a production-shaped first／restart Gate at exact
`-Xms512m -Xmx1536m -XX:+ExitOnOutOfMemoryError` while simultaneously retaining the full exact
counter envelope, 1,024 P4-C admissions, 65,536 raw roots, 4,096 journal targets, full
...
R2Q did not naturally exercise the integrated mutable-alias path. If the reviewed fixture selects
an integrated startup owner, the actual child must exercise it; otherwise a reviewed
machine-checked structural proof must lock same-owner disk exclusion, replacement rather than dual
hydration, no retained compressed／gzip state, and no second whole-tree copy. That proof compares no
object sizes and does not replace the actual selected-envelope child. P4-E0-B.10 fixes the E3
memory Gate at the sole
synchronous `ServerStartingEvent` production audit coordinate, where online players and selected
online owners are both exactly zero before login. The Gate still executes the production empty
online inventory path and preserves the 2,048／65,536 maxima through lifecycle-reachable sources;
...
- P4-E1: read-only scanner, projection, grouped audit, memory-only index, and completeness results;
  mutation and reclaim invocation are zero.
- P4-E2: login-only reconciliation after P4-D recovery; offline, Store, journal, and reclaim mutation
  are zero.
...
verified locally, committed, pushed, and remotely qualified. P4-E1-A is complete; the E1-B read-only
...
implementation remains suspended in its verified repaired backup after M1 qualification. A0.1 and
A0.2 are historical Stops at the nonpublic receiver and direct-coordinate authority Gates. The B.6
...
technical review is complete as historical research. P4-E0-B.8 supersedes B.7／A0.4 only for release
qualification and discontinues Candidate10–12 formal receipt infrastructure. P4-E2 product implementation
is locally implemented in its verified repaired backup with focused validation, while simplified release
qualification is ready but not started at 0／3. The remaining release product Gates and P4-E3 stay blocked;
no phase introduces chunk force, periodic/background scanning, network, or a second persistent truth. The completed
...
lease state identity, generation, and lease identity. Under B.9, close defaults to same-generation
`Incomplete(g)`; only exact `Completed(0)` with the complete source-unchanged zero-publication
proof returns to `CompleteIndex(g)`. Neither path republishes a permit. While active, audit and
E2 invalidation fail-fast with no state delta; server
...
CompleteIndexWithActiveLease(g) -> exact Completed(0) source-unchanged close -> CompleteIndex(g)
CompleteIndexWithActiveLease(g) -> every other close terminal -> Incomplete(g)
...
to `CompleteIndex(g)` only for exact `Completed(0)` with the complete source-unchanged proof;
every other close terminal demotes to `Incomplete(g)`. Neither outcome issues a new permit or
generation. One accepted E2 invalidation from any
...
## P4-E1-B2-B implementation and remote closure evidence
...
Candidate12 implementation-feasibility Stop. The prior requirement that the final no-replace
receipt link complete by H was not implementable with the allowed same-process macOS filesystem
operations: userspace could not distinguish an H-minus-epsilon link linearization from an
H-plus-epsilon linearization when both syscalls returned after H. B.7 changes that one boundary
only. It does not implement Candidate12, run a harness, open cold v3, select T1, or modify any
...
H strictly bounds COMMIT_READY and LINK_PREINVOKE_ELIGIBLE.
...
The reviewed T2 branch-aware 4.100-second ledger ends at COMMIT_READY preparation. H remains
5.750 seconds. Link latency is not represented as a bounded stage and cannot be absorbed by
increasing H, changing a watchdog, deleting ps／jcmd, weakening process identity, or reducing the
three-CLEAR requirement.

### Adjacent preinvoke eligibility and one publication

The sole LINK_PREINVOKE_ELIGIBLE check must be immediately adjacent to the syscall and prove all of:
...
No callback, logging, file read, allocation-heavy work, retry, yield, second process,
wrapper／DONE protocol, or another clock-dependent branch may occur between that check and the
syscall. A late check produces LINK_NOT_INVOKED and COMMIT_READY_DEADLINE_EXCEEDED with zero link
attempts.

After COMMIT_READY, the same supervisor may directly invoke exactly one same-filesystem, atomic,
no-replace link／linkat. Copy, replace-existing, rename-overwrite, retry, fallback API, wrapper
process, ln subprocess, DONE file／pipe, and a second publication path are forbidden. The exact
macOS／JDK route is intentionally unselected until the post-B.7 read-only technical review.

If COMMIT_READY and eligibility are both established no later than H, normal return may occur before
or after H. The only success coordinate is same-process normal return from that exact authorized
call, which is RECEIPT_COMMITTED. A crossed-H normal return is eligible to PASS and must not be
rejected by a later clock check. Reports must separate commit_ready_by_h,
link_preinvoke_eligible_by_h, receipt_link_normal_return, and receipt_link_crossed_h. They must not
claim transaction finished by H, receipt committed by H, or link completed within H unless the
return actually occurred by H, and even then timing is diagnostic rather than an extra
qualification requirement.

### Failure, orphan, post-link, and durability boundaries

Directly observed EEXIST, ENOENT, EXDEV, EACCES, EPERM, EIO, ENOSPC, EINTR, RuntimeException, or
other non-success return is RECEIPT_COMMIT_FAILED. Error and OOME must propagate with original
identity; they, process termination, external timeout, unknown syscall completion, or a possible link without observed same-process normal return
is RECEIPT_COMMIT_UNADJUDICATED. Neither state permits retry, alternate path, target readback,
stat／exists／open, reconciliation, backfill, cleanup-to-success, or later recovery.

Receipt-path existence alone is not restart authority. Restart scan, orphan adoption, readback
classification, later manifest reconciliation, and backfill are prohibited. A possible orphan
remains only in that failed attempt's unique namespace; the namespace is never reused and cannot
affect another run.

The final link is the observation's last persistent state-changing operation. After normal return,
only nonallocating local status propagation, normal control return, and normal process exit are
...
P4-E2 production implementation resumed or completed.
...
P4-E2-M1-D2-C12-A0.4 technical review = COMPLETE AS HISTORICAL TECHNICAL RESEARCH;
                                              SUPERSEDED FOR RELEASE QUALIFICATION
Candidate10–12 formal receipt track = DISCONTINUED AS OVER-COMPLEX NON-PRODUCT VERIFICATION INFRASTRUCTURE
P4-E0-B.8 qualification simplification amendment
= COMPLETE UPON THIS COMMIT'S UNIQUE EXACT-SHA ATTEMPT-1 REMOTE GATE PASS
P4-E2 product implementation       = LOCALLY IMPLEMENTED IN VERIFIED REPAIRED BACKUP;
                                      PRODUCT CODE COMPLETION NOT INVALIDATED BY HARNESS FAILURE
P4-E2 simplified release qualification = READY; NOT STARTED
simplified cold qualification      = 0/3
P4-C2 and later release Gates      = BLOCKED UNTIL SIMPLIFIED COLD 3/3 PASS
P4-E3                               = BLOCKED UNTIL P4-E2 IMPLEMENTATION AND RELEASE CLOSURE
P4-E                               = INCOMPLETE
```
