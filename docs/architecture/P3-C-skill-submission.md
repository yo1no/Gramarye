...
- P3-D owns the Store domain API, atomic compare-and-insert, formal revision allocation, commit conflict, and plan commit boundary. Store's greatest committed revision is the allocator truth; an Attachment latest pointer is not.
...
- Store SavedData and player Attachment updates are not one native transaction. P4 must define ordering, recovery, and reconciliation.
...
- `SkillOwnerId` is pure domain identity. Its lack of a Codec or StreamCodec is a P3-C phase boundary, not a permanent prohibition on a later ownership persistence design.
...
- `NOT_AUTHORIZED` and `QUOTA_EXCEEDED` are the bounded rejection vocabulary. C4 only preserves an existing rejection; it performs no quota lookup or admission decision.
...
- `SubmissionRevisionProposer` is the single proposal policy: New proposes revision zero, Existing proposes the successor, and the maximum revision produces exhaustion. A proposal is not formal allocation.
...
### Plan lifetime and deferred commit obligations

A plan is discarded after server restart, policy or descriptor/registry reload, commit conflict, failed reauthorization, owner mismatch, or authoritative quota rejection. It is never edited and retried. Before commit, composition must reacquire authority, match the plan owner, compare New/Existing state with the precondition, perform authoritative quota admission, and invoke Store compare-and-insert.

Before P3-D implementation, a read-only review must decide whether quota admission and precondition CAS share one atomic Store mutation boundary and define typed results for committed, conflict, quota rejection, and defensive revision exhaustion. Quota rejection must not be returned after a revision has already been inserted. Whether a later composition result reuses the Prepared warning report is also deferred; C4 does not claim commit-time quota semantics.
